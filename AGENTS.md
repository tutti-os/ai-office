...
- AI Doc and AI Slide create/edit flows, whether entered through CLI or the app UI, should resolve local agent providers in this order: explicit provider/runtime profile from a CLI argument or user selection, Tutti's global default agent provider from `tutti agent providers --json` `defaultProvider`, Codex if available, then Claude Code if available.
