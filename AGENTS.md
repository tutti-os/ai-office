...
- The doc app lives in `apps/doc` and runs as a local server app through `apps/doc/nextop.app.json` and `apps/doc/bootstrap.sh`.
...
- Nextop packaging utilities live in `@ai-app/nextop-packager`; app-specific package scripts should provide configuration and app branding only.
