  ...
  constructor() {
    super({
      ...
      buildEnv: (context, workspaceRoot) => ({
        ...
        AI_SLIDE_TOOL_BASE_URL: localToolBaseUrl(),
      }),
  ...
  return [
    {
      ...
      env: {
        AI_SLIDE_TOOL_BASE_URL: localToolBaseUrl(),
        AI_SLIDE_TOOL_TOKEN: context.toolAccess.token,
        AI_SLIDE_PROJECT_ID: context.project.id,
        AI_SLIDE_RUN_ID: context.run.id,
      },
    },
...
function localToolBaseUrl() {
  return process.env.AI_SLIDE_SERVER_URL ?? `http://127.0.0.1:${process.env.PORT ?? 8791}`;
}
