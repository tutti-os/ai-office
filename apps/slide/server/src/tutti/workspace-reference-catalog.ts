...
export function recordWorkspaceReference(input: {
  projectId: string;
  kind: string;
  absolutePath: string;
  displayName?: string;
  ...
  `).run(
    randomUUID(), input.projectId, input.kind, relativePath,
    input.displayName?.trim() || basename(input.absolutePath),
    input.description?.trim() || relativePath,
    input.mimeType, info.size, Math.trunc(info.mtimeMs), now, now,
  );
  return relativePath;
}
...
export function listWorkspaceReferenceRecords(projectId?: string): WorkspaceReferenceRecord[] {
  ...
  return rows<WorkspaceReferenceRow>(result).map((row) => ({
    id: row.id, projectId: row.project_id, kind: row.kind, relativePath: row.relative_path,
    displayName: row.display_name, description: row.description, mimeType: row.mime_type,
    sizeBytes: row.size_bytes, mtimeMs: row.mtime_ms, updatedAt: row.updated_at,
  }));
}
