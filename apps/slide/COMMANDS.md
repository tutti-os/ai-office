...
These commands expose AI PPT to Tutti apps and agents. Command outputs use `CliCommandOutput` envelopes.

## `slide status`

Return app health, project counts, runtime provider counts, and Tutti CLI availability for AI PPT.
...
Explicitly request Tutti to open one slide project in AI PPT. Use this only after the user confirms they want you to open the app directly; create, import, and agent-edit commands must not call it automatically.
...
Return local workspace paths, focused artifact paths, assets, exports, and guidance for one slide project in AI PPT. Local paths are for inspection; content modifications through CLI must trigger the app-owned agent run.
