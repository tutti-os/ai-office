...
import type { DocumentProject, DocumentType, LocalAgentProviderStatus, RuntimeProfile } from "@ai-document/shared";
import {
  allTemplatesLabel,
  gensparkTemplates,
                ...
                <div className="text-[12px] text-white/44">
                  {props.activePanel === "templates"
                    ? props.selectedCategory === allTemplatesLabel
                      ? `${gensparkTemplates.length} templates`
            ...
            {props.activePanel === "templates" ? (
              <div className="toolbar-scroll flex min-w-0 gap-2 overflow-x-auto pb-2">
                {props.categories.map((item) => {
                  ...
                  const count = item === allTemplatesLabel ? gensparkTemplates.length : props.templateCounts[item] ?? 0;
                  return (
                    <button
                      key={item}
