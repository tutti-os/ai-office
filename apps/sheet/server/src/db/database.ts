...
export function migrate(database: DatabaseSync) {
  database.exec(`
    ...
    CREATE TABLE IF NOT EXISTS sheet_run_events (
      id TEXT PRIMARY KEY,
      ...
      created_at TEXT NOT NULL,
      FOREIGN KEY (run_id) REFERENCES sheet_runs(id) ON DELETE CASCADE,
      FOREIGN KEY (project_id) REFERENCES projects(id) ON DELETE CASCADE
    );
    CREATE INDEX IF NOT EXISTS idx_sheet_runs_project ON sheet_runs(project_id, created_at);
    CREATE INDEX IF NOT EXISTS idx_sheet_run_events_run_order ON sheet_run_events(run_id, sort_order, created_at);
  ...
  ensureColumn(database, "sheet_runs", "agent_target_id", "TEXT");
