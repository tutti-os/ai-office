...
export function registerTuttiReferenceRoutes(server: FastifyInstance) {
  server.post<{ Body: ReferenceListRequest }>("/tutti/references/list", async (request) => {
    ...
    const filtered = filter ? items.filter((item) => item.type === "group" ? matchesGroupSearch(item, filter) : item.reference.displayName.toLowerCase().includes(filter)) : items;
  ...
  server.post<{ Body: ReferenceSearchRequest }>("/tutti/references/search", async (request) => {
    ...
    const filtered = references.filter((item) => matchesFilter(item.reference.displayName, filters)).filter((item) => !query || matchesReferenceSearch(item, query))
...
function loadProjectMetadata() { return new Map(rows<{ id: string; title: string; updated_at: string }>(getDb().prepare(`SELECT id, title, updated_at FROM projects`).all()).map((project) => [project.id, { title: project.title, updatedAt: project.updated_at }] as const)); }
