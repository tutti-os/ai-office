...
const baseUrl = process.env.AI_DOCUMENT_TOOL_BASE_URL ?? "http://127.0.0.1:8790";
const projectId = requiredEnv("AI_DOCUMENT_PROJECT_ID");
const token = requiredEnv("AI_DOCUMENT_TOOL_TOKEN");
...
async function handleRequest(method, params) {
  if (method === "initialize") {
    return {
      ...
      serverInfo: { name: "ai-document-tools", version: "0.1.0" },
      capabilities: { tools: {} },
    };
  }
...
async function requestJson(method, url, body) {
  const response = await fetch(url, {
    ...
    headers: {
      ...
      "x-ai-document-tool-token": token,
    },
  ...
  if (!response.ok) throw new Error(`ai-document tool gateway ${response.status}: ${JSON.stringify(data)}`);
  return data;
}
