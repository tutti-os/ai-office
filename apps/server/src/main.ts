...
import { AgentToolGateway } from "./documents/agent-tool-gateway.js";
import { AgentToolTokenStore, AgentToolUnauthorizedError } from "./documents/agent-tool-tokens.js";
import { DocumentRepository } from "./documents/document-repository.js";
import { DocumentService } from "./documents/document-service.js";
import { EventHub } from "./ws/event-hub.js";

const webDist = process.env.AI_DOCUMENT_WEB_DIST
  ? resolve(process.env.AI_DOCUMENT_WEB_DIST)
...
server.get("/api/health", async () => ({ ok: true, app: "ai-document" }));

server.get("/api/dev/fixtures/genspark-study-plan", async (request, reply) => {
  const fixturePath = process.env.AI_DOCUMENT_GENSPARK_TEST_HTML ?? "/Users/niuma/code/genspark/doc/test.html";
  try {
  ...
  return reply.type("text/html").send(`
    <html>
      <body style="font-family: system-ui; padding: 32px">
        <h1>ai-document server is running</h1>
...
try {
  ...
  server.log.info(`ai-document server listening on http://${host}:${port}`);
} catch (error) {
...
function readAgentToolCredential(request: { headers: Record<string, string | string[] | undefined>; query?: unknown }) {
  const header = request.headers["x-ai-document-tool-token"];
