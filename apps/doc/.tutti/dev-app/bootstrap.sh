...
if [ ! -f "$vite_cli" ]; then
  echo "AI Doc web dependencies are missing. Install workspace dependencies from $repo_root before loading this local app." >&2
  exit 1
fi
...
export AI_DOC_DEV_BACKEND_ORIGIN="$backend_origin"
...
echo "[ai-doc] web: http://$host:$port"
echo "[ai-doc] server: $backend_origin"
