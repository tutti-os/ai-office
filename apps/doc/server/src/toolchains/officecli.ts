...
import type { OfficeCliStatus } from "@ai-doc/shared";
import { appPaths } from "../local/paths.js";

const execFileAsync = promisify(execFile);
const officeCliVersion = process.env.AI_DOC_OFFICECLI_VERSION ?? "1.0.103";
...
export async function getOfficeCliStatus(): Promise<OfficeCliStatus> {
  const envPath = process.env.AI_DOC_OFFICECLI_PATH?.trim();
  if (envPath) {
...
export async function requireOfficeCli() {
  ...
  if (!status.available) {
    throw new Error(status.canInstall ? "OfficeCLI is required for DOCX. Download OfficeCLI from the home screen first." : (status.reason ?? "OfficeCLI is required for DOCX."));
  }
  return status;
}

export async function officeCliEnv(): Promise<Record<string, string>> {
  const status = await getOfficeCliStatus();
  if (!status.available || !status.executablePath) return {};
  return {
    OFFICECLI: status.executablePath,
    OFFICECLI_NO_AUTO_RESIDENT: "1",
    PATH: `${dirname(status.executablePath)}:${process.env.PATH ?? ""}`,
  };
}
...
function resolveOfficeCliAsset(sums: string) {
  if (process.env.AI_DOC_OFFICECLI_ASSET) return process.env.AI_DOC_OFFICECLI_ASSET;
