...
export function isHTMLElementInDocument(element: Element, doc: Document): element is HTMLElement {
  const ctor = doc.defaultView?.HTMLElement;
  return Boolean(ctor && element instanceof ctor);
}

export function isBackgroundImageObject(element: HTMLElement, doc: Document) {
...
export function updateBackgroundImageObject(doc: Document, image: ImageObjectElement, attributes: ImageAttributes) {
...
export function normalizeImageCssSize(value: string) {
...
export function cssUrlValue(url: string) {
